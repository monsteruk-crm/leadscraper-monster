...
Store ADRs in `docs/adr/` using the filename pattern `NNN-short-slug.md` (e.g. `001-vercel-fastapi-deployment.md`).

**When to write an ADR** — record a decision whenever you:
- Choose or replace a technology, framework, or hosting platform
- Change the database schema in a non-trivial way (new tables, significant new columns)
- Add a new external integration (API, service, queue)
- Make a deliberate trade-off that a future agent/developer might question
- Decide NOT to do something non-obvious

**ADR template** (use exactly these headings):
...
After creating an ADR, add a one-line entry to `docs/adr/README.md` (create it if absent) and link it from `docs/README.md`.
